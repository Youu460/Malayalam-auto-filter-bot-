...
async def start(client, message):    
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    f_caption=CUSTOM_FILE_CAPTION.format(query.from_user.mention,file_name= '' if title is None else title, file_size='' if size is None else size, file_caption='' if f_caption is None else f_caption)
                except Exception as e:
                    logger.exception(e)
                    f_caption=f_caption
            if f_caption is None:
    ...
    if not files_:
        ...
        try:
            ...
            if CUSTOM_FILE_CAPTION:
                try:
                    f_caption=CUSTOM_FILE_CAPTION.format(query.from_user.mention,file_name= '' if title is None else title, file_size='' if size is None else size, file_caption='')
                except:
                    return
